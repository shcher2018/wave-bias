function plot_wave_bias_ADCP(PL)
...
% get the "vector" analytical expression for the reference
PROC.ADCP = false;
WBv = analytical_wave_bias(z1,PL,PROC);
...
plot(WBv.net_bias_relative,z1,'b--','linewidth',.5);
plot(real(guw),z1,'g+');
plot(xlim,PL.z0*[1 1],'k--')
legend({'Stokes','Analytical bias','Without ADCP response','Simulation','Platform depth'},'location','SE');
xlabel('U_{bias} (m/s)')
ylabel('z (m)');
